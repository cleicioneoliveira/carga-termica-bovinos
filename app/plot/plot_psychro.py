...
Important:
    ``psychchart.load_chart_config()`` already returns a dictionary compatible
    with ``PsychChart(**chart_cfg)``. Do not unwrap the original YAML manually
    and do not pass a top-level ``chart`` key to ``PsychChart``.
"""
...
def _resolve_chart_config_path(cfg: dict[str, Any]) -> Path:
    ...
    Priority:
        1. cfg["psychchart_config"]
        2. cfg["chart_config"]
        3. cfg["chart"]["config"]
        4. app/plot/chart_config.yaml next to this file
    """
    candidates = [
        cfg.get("psychchart_config"),
        cfg.get("chart_config"),
    ]

    chart_block = cfg.get("chart")
    if isinstance(chart_block, dict):
        candidates.append(chart_block.get("config"))
    ...
    default_path = Path(__file__).resolve().parent / "chart_config.yaml"
    if default_path.exists():
        return default_path

    raise FileNotFoundError(
        "Could not find psychchart configuration file. "
        "Set cfg['psychchart_config'] or cfg['chart_config']."
    )
...
def _apply_nature_style_to_psychchart(ax, cfg: dict[str, Any]) -> None:
    ...
    # Refine psychchart background lines.
    ...
    # Refine text labels generated by psychchart.
...
def _convert_w_to_gkg_if_needed(points: np.ndarray, cfg: dict[str, Any]) -> np.ndarray:
    ...
    Many psychrometric routines use kg/kg internally, while the final academic
    figure is usually clearer in g/kg. This function supports both conventions.
    """
    moisture_unit = str(
        cfg.get("moisture_unit", cfg.get("w_unit", "g/kg"))
    ).lower()

    out = points.copy()

    # If the data look like kg/kg, convert to g/kg.
    # Example: 0.012 kg/kg -> 12 g/kg.
    if moisture_unit in {"kg/kg", "kg kg-1", "kg kg⁻¹"}:
        out[:, 1] *= 1000.0
        return out

    # Auto-detect conservative case: values below 0.2 are almost certainly kg/kg.
...
def plot_psychro(
    ...
    # Correct psychchart usage:
    # load_chart_config() already returns the payload expected by PsychChart.
    # Do not pass the raw YAML dictionary and do not unwrap a top-level "chart".
    chart = PsychChart(**chart_cfg)

    # Avoid duplicated titles. PsychChart applies cfg.title internally during
    # draw(); the final title is applied later by this pipeline.
